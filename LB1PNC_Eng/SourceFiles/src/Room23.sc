			...
			(
				(and
					...
					(not (& (event modifiers?) emRIGHT_BUTTON))
						...
						(if (ClickedInRect 4 313 186 189 event) ;exit room down
					(event claimed: TRUE)
					(switch theCursor
						(999

							(ego setMotion: MoveTo (ego x?) (+ (ego y?) 200))
;;;							
						)
						(else
							(event claimed: FALSE)
					...
					(if (ClickedInRect 0 5 95 124 event) ;exit room left
					(event claimed: TRUE)
					(switch theCursor
						(999
...
;;;							
				...
				(if (ClickedInRect 0 5 154 189 event) ;exit room left2
					(event claimed: TRUE)
					(switch theCursor
						(999
...
;;;							
					...
					(if (ClickedInRect 313 319 73 189 event) ;exit room right
					(event claimed: TRUE)
					(switch theCursor
						(999
...
;;;													#button {Entrar} 6
					...
					(switch theCursor
						...
						(998
							(Print {It's the crank to lift the bucket from the well.})
						)
